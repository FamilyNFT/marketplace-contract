...
contract LSP8MarketplaceOffer is LSP8MarketplaceSale {
    ...
    modifier LSP8OfferDoesNotExist(
        ...
        require(
            !ILSP8IdentifiableDigitalAsset(offerLSP8Address).isOperatorFor(
                address(this),
                offerTokenId
            ),
    ...
    modifier LSP8OfferExistsForThisLSP8(
        ...
        require(
            _offers[LSP8Address][tokenId]
                .LSP8TokenIds[offerLSP8Address]
                .contains(offerTokenId),
    ...
    modifier offerCreatorHasEnoughLSP7Balance(
        ...
        require(
            ILSP7DigitalAsset(LSP7Address).balanceOf(msg.sender) >
                _offers[LSP8Address][tokenId].LSP7Amounts[offerCreator].get(
                    LSP7Address
                ),
            "Sender doesn't have enough token balance."
        );
    ...
    modifier LSP7OfferExists(
        ...
        require(
            _offers[LSP8Address][tokenId].LSP7Amounts[offerCreator].contains(
                LSP7Address
            ),
    ...
    function _makeLSP8Offer(
        ...
        ILSP8IdentifiableDigitalAsset(offerLSP8Address).authorizeOperator(
            address(this),
            offerTokenId
        );
    }
    ...
    function _removeLSP8Offer(
        ...
        _offers[LSP8Address][tokenId].LSP8TokenIds[offerLSP8Address].remove(
            offerTokenId
        );
        ILSP8IdentifiableDigitalAsset(offerLSP8Address).revokeOperator(
            address(this),
            offerTokenId
        );
    }
    ...
    function _returnLSP8OfferTokenIdsByAddress(
        ...
        for (
            uint256 i = 0;
            i < _offer.LSP8TokenIds[offerLSP8Address].length();
            i++
        ) {
    ...
    function _makeLSP7Offer(
        ...
        ILSP7DigitalAsset(LSP7Address).authorizeOperator(
            address(this),
            LSP7Amount
        );
        ...
        returns (address[] memory, uint256[] memory)
    {
        ...
        return (
            _offers[LSP8Address][tokenId].LYXOfferCreators.values(),
            LYXAmounts
        );
    }
